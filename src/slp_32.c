...
static uint32_t lcd_buf[BUF_SIZE];
static uint32_t current_size = 0;
static uint dma_chan = 0;

static volatile Packet word_buf;
uint32_t word_buf_c = 0;

static void lcd_write_last_word();


static inline bool lcd_set_color_rgb(ColorRGBByte* c, bool r, bool g, bool b) {
    c->r = r;
    c->r0 = r;
    c->g = g;
    c->g0 = g;
...
static inline void lcd_write_last_word() {
    if (word_buf_c != 0) {
        lcd_buf[current_size++] = word_buf.word;
        word_buf.word = 0;
        word_buf_c = 0;
    }
}

static inline void lcd_write(bool dc, const uint8_t data) {
...
static inline void lcd_write_pixel_3b(bool r, bool g, bool b) {
    ColorRGBByte c;
    lcd_set_color_rgb(&c, r, g, b);
    lcd_write(1, c.color);
}


static inline void draw_pixel_18b() {
    ...
    while (1) {
        ...
        for (int y = 0; y < SCREEN_HEIGHT; ++y) {
            ...
            for (int x = 0; x < SCREEN_WIDTH; ++x) {
                ...
                //lcd_write(true, colour >> 8);
                //lcd_write(true, colour);
                lcd_write_pixel_3b(colour & 1, colour & 2, colour & 4);
            }
